...
class PaperTradingEngine:
    ...
    def update_open_positions(self, candle: Candle) -> int:
        """Advance open positions without allowing new entries."""
        updated = 0
        timeframe_trackers = self.trackers.get(candle.symbol, {}).get(candle.timeframe.value, {})

        for tracker in timeframe_trackers.values():
            try:
                tracker.strategy._candle_buffer.append(candle)
                if len(tracker.strategy._candle_buffer) > 1000:
                    tracker.strategy._candle_buffer = tracker.strategy._candle_buffer[-500:]

                tracker.strategy._check_stops(candle)
                tracker.record_equity(candle.timestamp)
                updated += 1
            except Exception as e:
                logger.error(f"[PAPER] Open-position update error: {e}")
    ...
    def get_completed_trades(
        ...
        """Get closed trades across all strategies."""
        trades = []

        for tracked_symbol, timeframe_dict in self.trackers.items():
            if symbol and tracked_symbol != symbol:
                continue

            for tracked_timeframe, strategy_dict in timeframe_dict.items():
                if timeframe and tracked_timeframe != timeframe.value:
                    continue

                for strategy_name, tracker in strategy_dict.items():
                    for trade in tracker.strategy.trades:
                        trades.append({
                            **trade,
                            "strategy_name": strategy_name,
                            "timeframe": tracked_timeframe,
                        })
